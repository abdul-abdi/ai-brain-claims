...
thread: "memory"
date: "2026-05-09"
description: "Effective working-memory capacity in transformer LLMs degrades at roughly 7±2 simultaneously-tracked chunks when measured under properly controlled conditions, mirroring Miller's classic limit. Strong form (a sharp 7±2 cliff) is refuted by N-back data showing continuous logarithmic decline; weak form (effective capacity far below nominal, 4-8 reliably-tracked entities) is supported."
tags: ["working-memory", "miller", "cowan", "context-window", "transformer-internals"]
---
...
The analogy is thought-provoking scaffolding for agent system design, but it should not be treated as an established empirical finding.
...
- **Design implication for agent systems:** Even under the contested verdict, the finding that effective utilization is 10-20% of nominal (BABILong) should drive agent system design. If you're building a context-window-as-working-memory system, plan for ~4-8 reliably-tracked entities in the active context, not the nominal token count. This is a practical engineering prior even without mechanistic proof.
